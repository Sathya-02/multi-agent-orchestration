...
export default function AgentEditorPanel({
  ...
  const canEdit      = can(user, 'edit_agent')
  const canApprove   = can(user, 'approve_spawn')
  const canSkillsMd  = can(user, 'edit_skills_md')

  const TABS = [
    { key: 'list',   label: '🤖 Agents' },
    ...(canCreate   ? [{ key: 'create',  label: '✨ Create' }] : []),
    ...(canSkillsMd ? [{ key: 'skills',  label: '📝 Skills MD' }] : []),
    ...(canApprove && pendingSpawns?.length > 0 ? [{ key: 'spawns', label: `⏳ Spawns (${pendingSpawns.length})` }] : []),
  ]
  ...
  return (
    <div className="overlay-panel agent-editor-panel">
      <div className="overlay-header">
        <span>🤖 Agents <span style={{ fontSize:11, color:'var(--tx-muted)', fontWeight:400 }}>({agents.length})</span></span>
      ...
      {tab === 'list' && (
        <div style={{ flex:1, overflowY:'auto', padding:'10px 14px' }}>
          {agents.length === 0 && <div className="empty-hint">No agents yet. {canCreate ? 'Use Create tab to add one.' : ''}</div>}
          {agents.map((a, i) => {
              ...
              <div key={id || i} style={agentCard}>
                {isEditing ? (
                  <div style={{ flex:1, display:'flex', flexDirection:'column', gap:5 }}>
                    <input className="topic-input" value={editFields.name}
                      onChange={e => setEditFields(p => ({ ...p, name: e.target.value }))}
                      placeholder="Name" style={{ marginBottom:0 }} />
                    <input className="topic-input" value={editFields.role}
                      onChange={e => setEditFields(p => ({ ...p, role: e.target.value }))}
                      placeholder="Role / instructions" style={{ marginBottom:0 }} />
                    <input className="topic-input" value={editFields.model}
                      onChange={e => setEditFields(p => ({ ...p, model: e.target.value }))}
                      placeholder="Model" style={{ marginBottom:0 }} />
                  ...
                  <>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontSize:13, fontWeight:600, color:'var(--tx-primary)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{a.name}</div>
                    ...
                    {canEdit && (
                      <button className="agent-action-btn" onClick={() => startEdit(a)} title="Edit agent">✏️</button>
                    )}
                    {canDelete && (
                      <button className="del-btn" onClick={() => handleDeleteAgent(id)} title="Delete agent">🗑</button>
                    )}
                  </>
                )}
              </div>
          ...
          {!canEdit && !canCreate && (
            <div style={{ ...viewerBanner, marginTop:8 }}>🔒 Agents are read-only for your role.</div>
          )}
        </div>
      )}
      ...
      {tab === 'create' && canCreate && (
        <div className="kb-body">
          ...
          <input className="topic-input" placeholder="Agent name *" value={newName}
            onChange={e => setNewName(e.target.value)} style={{ marginBottom:6 }} />
          <input className="topic-input" placeholder="Role / system instructions" value={newRole}
            onChange={e => setNewRole(e.target.value)} style={{ marginBottom:6 }} />
          <input className="topic-input" placeholder="Model (optional, e.g. llama3)" value={newModel}
            onChange={e => setNewModel(e.target.value)} style={{ marginBottom:10 }} />
          <button className="run-btn" onClick={onCreateClick} disabled={!newName.trim()}>✨ Create Agent</button>
        </div>
      )}
        ...
        <div style={{ padding:'10px 14px' }}>
          ...
          {pendingSpawns.map((s, i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8,
              padding:'8px 10px', borderRadius:7, background:'rgba(255,255,255,0.03)', border:'1px solid var(--bd-subtle)' }}>
              <span style={{ flex:1, fontSize:12, color:'var(--tx-secondary)' }}>{s.name || s.agent_name || `Spawn #${i+1}`}</span>
              {canApprove ? (
                <>
